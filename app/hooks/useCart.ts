...
export function useCart() {
  ...
  // Add item to cart
  const addItem = useCallback(async (productId: string | null, medicineId: string | null, quantity: number) => {
    try {
      setError(null);
  ...
  const addToCart = useCallback(({ productId, medicineId, quantity = 1 }: { productId?: string; medicineId?: string; quantity?: number }) => {
