...
const Header = () => {
  ...
  const checkLoginStatus = useCallback(async () => {
    try {
      // Dispatch Redux actions to check auth and get user details
      const authResult = await dispatch(checkAuthStatus()).unwrap();

      if (authResult.loggedIn && authResult.userDetails) {
        await dispatch(getUserDetails());
        setPhoneNumber(authResult.userDetails.phoneNumber || '');
        setIsAdmin(authResult.userDetails.role?.toUpperCase() === 'ADMIN');
        fetchCartCount();
      } else {
        setPhoneNumber('');
        setIsAdmin(false);
  ...
  useEffect(() => {
    if (!isLoginModalOpen && isLoggedIn === false) {
      checkLoginStatus();
    }
  }, [isLoginModalOpen, checkLoginStatus, isLoggedIn]);
  ...
  // Initial load - sync with Redux state
  useEffect(() => {
    if (isLoggedIn && user) {
